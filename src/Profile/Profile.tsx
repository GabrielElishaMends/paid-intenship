...
const Profile = () => {
    ...
    const fetchCompanies = async () => {
      if (currentUser) {
        ...
        querySnapshot.forEach((doc) => {
          companyData.push({ ...doc.data(), id: doc.id, showFullDescription: false } as CompanyInfoWithDescription);
        });
  ...
  const handleDeleteClick = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this company information?')) {
      try {
  ...
  const toggleDescription = (id: string) => {
    setCompanies((prevCompanies) =>
      prevCompanies.map((company) =>
        company.id === id ? { ...company, showFullDescription: !company.showFullDescription } : company
      )
    );
  };
      ...
      {userRole === 'company' && (
        <>
          ...
          {loading ? (
            ...
            companies.length === 0 ? (
              <p className="text-danger">You have not added any Internship Application</p>
            ) : (
                    ...
                    <Row>
                      <Col className="mb-1">
                        ...
                        {company.showFullDescription ? (
                          <>
                            {company.description}
                            <Button variant="link" onClick={() => toggleDescription(company.id)} style={{ color: 'white' }}>See less</Button>
                          </>
                        ) : (
                          <>
                            ...
                            {company.description.length > 100 && (
                              <Button variant="link" onClick={() => toggleDescription(company.id)} style={{ color: 'white' }}>See more</Button>
                            )}
                          </>
                        )}
                      </Col>
